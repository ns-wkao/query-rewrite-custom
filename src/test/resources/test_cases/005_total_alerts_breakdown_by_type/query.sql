...
WHERE ((( alert_event.timestamp ) >= (TIMESTAMP '2025-01-03') AND ( alert_event.timestamp ) < (TIMESTAMP '2025-01-17'))) AND (alert_event.alert_type ) IN ('Compromised Credential', 'DLP', 'Malware', 'Security Assessment', 'anomaly', 'malsite', 'policy') AND (alert_event.ns_tenant_id ) = 2683 AND (-- For userip field, apply CIDR range filter if specified
           1=1
          AND

WITH alert_event AS (
      ...
      FROM (
       SELECT A.* ,
        ...
        AND ((( A.timestamp ) >= ((DATE_ADD('second', -29, NOW()))) AND ( A.timestamp ) < ((DATE_ADD('second', 30, DATE_ADD('second', -29, NOW())))))))
    )
SELECT
...
WHERE ((((( alert_event.timestamp )) >= ((DATE_ADD('second', -29, NOW()))) AND (( alert_event.timestamp )) < ((DATE_ADD('second', 30, DATE_ADD('second', -29, NOW()))))))) AND (alert_event.ns_tenant_id ) = 2683 AND (
           1=1
          AND
           1=1
          AND
